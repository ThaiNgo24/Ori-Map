        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 custom-scrollbar">
          ...
          {loading && (
            <div className="flex gap-4 justify-start animate-pulse">
              <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center"><Bot className="w-6 h-6 text-indigo-300" /></div>
              <div className="p-4 bg-white border border-slate-200 rounded-2xl text-slate-400 text-sm">AI đang suy nghĩ...</div>
            </div>
          )}
        </div>

        {/* Input area */}
        <div className="p-6 md:p-10 bg-gradient-to-t from-slate-50 via-slate-50 to-transparent">
          <div className="max-w-4xl mx-auto relative">
            <textarea
              rows={1}
              className="w-full p-4 pr-16 rounded-2xl border border-slate-200 bg-white shadow-lg focus:ring-4 ring-indigo-100 outline-none resize-none text-slate-800"
            ...
            <button 
              ...
              className="absolute right-3 top-3 w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center hover:bg-indigo-700 transition-all disabled:opacity-50"
            >
              <Send className="w-5 h-5" />
            </button>
          </div>
          <p className="text-center text-[10px] text-slate-400 mt-4">Orie Map Career Mentor AI có thể đưa ra câu trả lời không chính xác 100%. Hãy tham khảo thêm ý kiến chuyên gia.</p>
        </div>
      </main>
    </div>
  );
