...
const Dashboard: React.FC<DashboardProps> = ({ savedMajorIds, userProfile, onUpdateProfile, onLogout, onNavigate }) => {
  ...
  const renderOverview = () => (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-red-50/80 backdrop-blur-sm border border-red-100/60 p-8 rounded-[2.5rem] flex flex-col md:flex-row items-center gap-8 shadow-sm">
        <div className="w-16 h-16 bg-red-100 rounded-[1.5rem] flex items-center justify-center text-red-600 flex-shrink-0">
          <AlertTriangle className="w-8 h-8" />
        </div>
        <div className="flex-1 text-center md:text-left">
          <h4 className="font-[800] text-xl text-red-900 mb-1">Cảnh báo: Tiến độ ôn thi sụt giảm</h4>
          <p className="text-red-700/80 font-medium leading-relaxed">Thời gian luyện đề Toán của bạn giảm 40% so với tuần trước. Bạn cần bứt phá để đạt mục tiêu 9.0.</p>
        </div>
        <button className="px-8 py-4 bg-red-600 text-white rounded-2xl font-black text-sm hover:bg-red-700 transition-all shadow-lg shadow-red-500/20 whitespace-nowrap">Giải pháp AI</button>
      </div>

      <div className="grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 bg-[#111E6C] rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl shadow-blue-900/20">
          <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-white opacity-[0.03] rounded-full blur-[80px] -mr-[200px] -mt-[200px]"></div>
          <div className="relative z-10 flex flex-col h-full justify-between gap-12">
            <div className="flex justify-between items-start">
              <div className="space-y-4">
                <span className="bg-white/10 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border border-white/10">Mục tiêu ưu tiên</span>
                <h3 className="text-4xl font-[900] leading-tight">
                  {userSavedMajors.length > 0 ? userSavedMajors[0].majorName : 'Chưa chọn ngành mục tiêu'}
                  <br />
                  {userSavedMajors.length > 0 ? userSavedMajors[0].university : 'Vui lòng khám phá ngành học'}
                </h3>
              </div>
              <div className="w-20 h-20 rounded-full border-4 border-white/10 flex items-center justify-center text-3xl font-black">65%</div>
            </div>
            <div className="space-y-4">
              <div className="flex justify-between font-bold text-sm text-blue-100/60">
                <span>Trình độ hiện tại: Khá+</span>
                <span>Cần thêm: 15% nỗ lực</span>
              </div>
            ...
            <button 
              ...
              className="flex items-center gap-3 text-sm font-black text-blue-100 hover:text-white transition-colors"
            >
              Xem chi tiết lộ trình <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>

        <div className="bg-white rounded-[3rem] p-10 border border-slate-200/60 shadow-xl shadow-slate-200/20 flex flex-col justify-between space-y-8">
          <div className="space-y-1">
            <h4 className="text-xl font-[900] text-slate-900">Thống kê học tập</h4>
            <p className="text-slate-400 text-sm font-bold uppercase tracking-widest">Tháng 05</p>
          </div>
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 text-center space-y-1">
              <p className="text-3xl font-[900] text-[#111E6C]">12</p>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Chuyên đề</p>
            </div>
            <div className="bg-green-50 p-6 rounded-3xl border border-green-100 text-center space-y-1">
              <p className="text-3xl font-[900] text-green-600">85%</p>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Hoàn thành</p>
            </div>
          </div>
          <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
             ...
             <span className="text-xs font-bold text-slate-400">Đứng thứ #14 khối</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] lg:h-[calc(100vh-6rem)] bg-[#F8FAFC]">
      <aside className="w-80 bg-white border-r border-slate-200/60 hidden xl:flex flex-col p-8 space-y-12">
        <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-3xl border border-slate-100">
          <div className="w-12 h-12 rounded-2xl bg-[#FF8C00] flex items-center justify-center text-white shadow-lg shadow-orange-500/20">
            <UserCircle className="w-7 h-7" />
          </div>
          <div>
            <p className="text-sm font-black text-slate-900 leading-tight">{userProfile.name}</p>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Học sinh {userProfile.grade}</p>
          </div>
        </div>
        ...
        <nav className="space-y-3">
          <button 
            ...
            className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[15px] font-bold transition-all duration-300 ${
              activeTab === 'overview' 
        ...
        <div className="mt-auto p-6 bg-gradient-to-br from-[#111E6C] to-[#4F46E5] rounded-[2.5rem] text-white space-y-4 shadow-xl">
           <p className="text-xs font-black uppercase tracking-widest opacity-60">Gói Pro</p>
           <p className="text-lg font-bold">Mở khóa full tiềm năng AI</p>
           <button 
             onClick={() => setActiveTab('upgrade')}
             className="w-full py-3 bg-white text-[#111E6C] rounded-2xl font-black text-sm hover:scale-105 transition-transform"
           >
             Nâng cấp
           </button>
        </div>
          ...
          <header className="flex justify-between items-end">
            <div className="space-y-2">
              ...
              <p className="text-slate-500 font-medium text-lg">
                {activeTab === 'overview' ? 'Thứ Hai, 24 Tháng 05, 2024' : 
                 activeTab === 'settings' ? 'Quản lý tài khoản của bạn' : 
                 activeTab === 'exams' ? 'Tự tin chinh phục mọi thử thách' : 
                 activeTab === 'majors' ? 'Nơi những lựa chọn trở thành tương lai' : 
                 activeTab === 'roadmap' ? 'Nâng cấp kỹ năng AI để làm chủ kỷ nguyên mới' : 'Trở thành phiên bản tốt nhất cùng Orie Map PRO'}
              </p>
            </div>
            <div className="flex gap-4">
               <button className="relative p-4 bg-white rounded-2xl border border-slate-200 text-slate-400 hover:text-[#4F46E5] transition-all shadow-sm">
