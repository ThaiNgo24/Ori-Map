    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-20 text-center">
        <div className="space-y-4 mb-10">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 tracking-tighter">
        ...
        <div className="relative group max-w-4xl mx-auto" ref={suggestionRef}>
          <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-orange-400 rounded-[2.5rem] blur opacity-15 group-focus-within:opacity-30 transition duration-1000 group-focus-within:duration-200"></div>
          ...
          <div className="relative">
            ...
            <input
              type="text"
              className="block w-full pl-16 pr-10 sm:pr-40 py-6 bg-white border border-slate-200/60 rounded-[2.5rem] shadow-2xl shadow-indigo-100/50 focus:ring-8 focus:ring-indigo-50/50 focus:border-indigo-500 transition-all text-xl outline-none font-bold text-slate-800 placeholder:text-slate-300 z-0"
              placeholder="Bạn muốn học gì? Nhập từ khóa hoặc hỏi Orie AI..."
              value={searchTerm}
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <aside className="lg:w-80 flex-shrink-0">
          <div className="bg-white p-8 rounded-[3rem] border border-slate-200/60 shadow-sm sticky top-28 space-y-10">
            <div className="flex items-center justify-between border-b border-slate-100 pb-6">
              <h3 className="font-black text-lg flex items-center gap-3">
                <SlidersHorizontal className="w-5 h-5 text-indigo-600" /> Bộ lọc
              </h3>
              <button 
        ...
        <main className="flex-1">
          {sortedFilteredMajors.length > 0 ? (
            <div className="grid md:grid-cols-2 xl:grid-cols-2 gap-8 animate-in fade-in duration-500">
              {sortedFilteredMajors.map((major) => {
                const isFavorited = savedMajorIds.includes(major.id);
                return (
                  <div 
                    key={major.id} 
                    className={`bg-white rounded-[2.5rem] border p-8 shadow-sm hover:shadow-xl transition-all group flex flex-col border-b-4 ${
                      isFavorited 
                  ...
                  >
                      ...
                      <div className="space-y-2 w-full">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex gap-1">
                            {major.demand === 'high' && (
                              <span className="px-2 py-1 bg-green-50 text-green-600 text-[9px] font-bold rounded border border-green-100 uppercase">Cầu cao</span>
                            )}
                          </div>
                        </div>
                        <h3 className="text-2xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors leading-tight">{major.majorName}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      <div className="bg-white p-4 rounded-2xl border border-slate-100 group-hover:bg-indigo-50/30 transition-colors">
                        <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest block mb-1">Điểm chuẩn</span>
                        <p className="text-2xl font-black text-indigo-600">{major.score.toFixed(1)}</p>
                      </div>
                      <div className="bg-white p-4 rounded-2xl border border-slate-100 group-hover:bg-indigo-50/30 transition-colors">
                        <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest block mb-1">Học phí TB</span>
                        <p className="text-sm font-bold text-slate-700 truncate">{major.tuition}</p>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-2 mb-10">
                      {major.blocks.map(b => (
                        <span key={b} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] font-black tracking-widest group-hover:border-indigo-200 transition-colors">
                          {b}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="mt-auto pt-6 border-t border-slate-100 flex gap-3">
                      <button 
                        ...
                        className="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-slate-900/10"
                      >
                        Chi tiết <ArrowUpRight className="w-4 h-4" />
                      </button>
                      <button 
                        onClick={() => onToggleSave(major.id)}
                        className={`px-5 py-4 border-2 rounded-2xl transition-all active:scale-95 ${
                          isFavorited 
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-[3rem] border border-slate-100 border-dashed animate-in fade-in duration-500">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-2">Không tìm thấy ngành phù hợp</h3>
